...
import { fbImportAll, fbCatBatchWrite } from '../services/firebaseDB.js';
    ...
    try {
      ...
      for (const entity of ENTITIES) {
        ...
        try {
          ...
          const isCategorySpecific = ['trials', 'projects', 'formulations', 'ingredients', 'blocks'].includes(entity.key);
          let totalWritten = 0;
          ...
          if (isCategorySpecific) {
            ...
            taggedItems.forEach(item => {
              const cat = item.Category || 'herbicide';
            ...
            for (const [cat, catItems] of Object.entries(grouped)) {
              const chunks = [];
              for (let i = 0; i < catItems.length; i += 400) {
                chunks.push(catItems.slice(i, i + 400));
              }
              for (const chunk of chunks) {
                const res = await fbCatBatchWrite(cat, entity.key, chunk, uid);
                totalWritten += res.count || chunk.length;
              }
            }
          } else {
            // Shared collections (e.g., organisations)
            const chunks = [];
            for (let i = 0; i < taggedItems.length; i += 400) {
              chunks.push(taggedItems.slice(i, i + 400));
            }
            for (const chunk of chunks) {
              const res = await fbImportAll({ [entity.key]: chunk }, uid);
              totalWritten += res.results?.[entity.key]?.count || chunk.length;
            }
          }
