...
export default function PhotoAnalyzerView({
  ...
  // Redraw when results, opacity, highlightedIndex, or image loading changes
  useEffect(() => {
    ...
    results.forEach((item, index) => {
      ...
      // Select different colors based on weed index or names
      ...
      const label = `${item.name} (${item.cover}%)`;
  ...
  }, [isOpen, imgLoaded, results, opacity, highlightedIndex]);
  ...
  const handleCopy = () => {
    ...
    const text = results.map(w => `${w.name} (${w.commonName || ''}): ${w.cover}% cover, ${w.growthStage || ''}`).join('\n');
        ...
        <div className="w-full md:w-80 border-t md:border-t-0 md:border-l border-slate-100 flex flex-col justify-between bg-slate-50/50">
          ...
          <div className="p-4 border-b border-slate-100 flex items-center justify-between bg-white">
            <div className="flex items-center gap-2">
              ...
              <div>
                <h3 className="font-bold text-slate-800 text-sm">Photo AI Analyzer</h3>
                <p className="text-[10px] text-slate-400">Object Bounding & Percentages</p>
              </div>
            </div>
            <button
              onClick={onClose}
          ...
          <div className="flex-1 overflow-y-auto p-4 space-y-3">
            {loading ? (
                  ...
                  >
                    <div className="flex justify-between items-start gap-2">
                      ...
                      <div className="text-right shrink-0">
                        <p className="text-xs font-bold text-emerald-700">{item.cover}% cover</p>
              ...
              <div className="text-center py-12 text-slate-400">
                <p className="text-xs">No analysis results yet.</p>
              </div>
            )}
          </div>
          ...
          <div className="p-4 border-t border-slate-100 bg-white space-y-4">
            ...
            <div className="flex gap-2">
              ...
              {onApplyValue && results && results.length > 0 && (
                <button
                  onClick={() => {
                    const totalCover = results.reduce((acc, curr) => acc + (curr.cover || 0), 0);
                    onApplyValue(Math.min(100, totalCover));
                  }}
