...
export const CATEGORIES = {
  ...
  fungicide: {
    ...
    aiPhotoPrompt: 'Act as a senior plant pathologist and expert diagnostic taxonomist. Perform high-precision computer-vision visual diagnosis of this fungicide trial plot photo. 1) Identify all fungal, bacterial, or viral plant diseases and pathogens visible on leaves, stems, or fruits. 2) Name each identified target using standard plant pathology taxonomy: "Common Name (Scientific Name)" (e.g., "Late Blight (Phytophthora infestans)", "Early Blight (Alternaria solani)", "Powdery Mildew (Erysiphe necator)", "Downy Mildew (Pseudoperonospora cubensis)", "Rice Blast (Pyricularia oryzae)", "Anthracnose (Colletotrichum spp.)", "Septoria Leaf Spot (Septoria lycopersici)", "Leaf Rust (Puccinia spp.)", "Bacterial Spot (Xanthomonas spp.)"). 3) Analyze visual lesion morphology: inspect concentric rings, halo chlorosis, water-soaked margins, sporulation patches, and necrotic flecks. 4) Estimate disease severity percentage (0-100%) and disease incidence percentage based strictly on visible evidence. 5) Evaluate green leaf area (%) and calculate disease control efficiency with scientific rigor. If a symptom is ambiguous, specify lower confidence percentage rather than misclassifying.',
  ...
  pesticide: {
    ...
    aiPhotoPrompt: 'Act as a senior agricultural entomologist and expert pest diagnostic taxonomist. Perform high-precision computer-vision visual diagnosis of this pesticide trial plot photo. 1) Identify all crop pest insects, mites, larvae, or arthropods present, as well as characteristic pest damage. 2) Name each identified target using standard entomological taxonomy: "Common Name (Scientific Name)" (e.g., "Fall Armyworm (Spodoptera frugiperda)", "Whitefly (Bemisia tabaci)", "Cotton Aphid (Aphis gossypii)", "Brown Planthopper (Nilaparvata lugens)", "Two-Spotted Spider Mite (Tetranychus urticae)", "Chilli Thrips (Scirtothrips dorsalis)", "Rice Stem Borer (Scirpophaga incertulas)", "Mealybug (Planococcus citri)", "Flea Beetle (Epitrix spp.)", "Bollworm (Helicoverpa armigera)"). 3) Analyze visual feeding damage patterns: inspect hole chewing, leaf skeletonization, windowpane damage, stippling/bronzing, leaf curling, leaf mines, frass deposits, and honeydew/sooty mold. 4) Estimate pest count per plant/unit, live vs dead count, feeding damage percentage (0-100%), and crop damage rating (0-9 scale). 5) Detect beneficial predatory insects (ladybugs, lacewings, parasitoid wasps) if present. If an insect is ambiguous, specify lower confidence percentage rather than misclassifying.',
